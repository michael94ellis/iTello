...
@IBDesignable public final class JoyStick: UIView {

    /// Receives updates as the joystick is moved around
    /// Supports polar and cartesian reporting.
    /// The function to call with a position report is held in the enumeration value.
    public var monitor: JoyStickMonitor = .none
...
extension JoyStick {
    ...
    private func initialize() {
        ...
        let bundle = Bundle(for: JoyStick.self)
    ...
    /**
     Report the current joystick values to the registered `monitor`.
     */
    private func reportPosition() {
        ...
        switch monitor {
        case let .polar(monitor): monitor(JoyStickPolarReporter(angle: self.angle, displacement: displacement))
        case let .xy(monitor): monitor(JoyStickXYReporter(x: delta.dx, y: -delta.dy))
        case .none: break
        }
    }
